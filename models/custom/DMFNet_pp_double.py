...
class DMFNet_pp_double(nn.Module):
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='bn', num_classes=4):
        ...
        self.residual1 = nn.Sequential(
            MFunit(n, n * 2, g=groups, stride=1, norm=norm),
            MFunit(n * 2, n, g=groups, stride=1, norm=norm),
        )
        self.residual2 = nn.Sequential(
            MFunit(channels, channels * 2, stride=1, norm=norm),
            MFunit(channels * 2, channels, stride=1, norm=norm)
        )
        self.residual3 = nn.Sequential(
            MFunit(channels * 2, channels * 3, stride=1, norm=norm),
            MFunit(channels * 3, channels * 2, stride=1, norm=norm)
        )
    ...
    def forward(self, x):
        ...
        y2 = torch.cat([x2_res, y2], dim=1)
        y2 = self.decoder_block2(y2)

        y3 = self.upsample3(y2)  # H//2
        y3 = torch.cat([x1_res, y3], dim=1)
