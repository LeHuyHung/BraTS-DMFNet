...
class DMFNet_multiscale(nn.Module):
    ...
    def forward(self, x):
        # 4 inputs: /1, /2, /4, /8
        x01 = x
        x02 = self.conv(interpolate(x, scale_factor=1/2))
        x04 = self.conv(interpolate(x, scale_factor=1/4))
        x08 = self.conv(interpolate(x, scale_factor=1/8))
